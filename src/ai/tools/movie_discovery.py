...
def search_movies(query: str, limit: int = 5, config: RunnableConfig = {}):
    ...
    if not configurable:
        raise ValueError("Configurable or metadata must be provided in the config.")
    ...
    try:
        total_results = int(response_data.get("total_results"))
    except:
        total_results = -1
    if total_results == 0:
        return []
    ...
    results = response_data.get("results")[:limit]
